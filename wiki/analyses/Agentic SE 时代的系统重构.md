...
| 问题 | 现状（2026-09-23） |
|---|---|
| spec 是否是新的源代码 | 旗手是 2025 年的 Tessl（已转型）；GitHub 的 Spec Kit 方法论文档在理念上接近 spec-as-source，但工具实际形态是 spec-first，持久化文档也声明三种模型都不是默认；Böckeler 担心 spec-as-source 兼有僵化与非确定性。SDD 的收益证据见 [[SDD 收益主张的实证赤字]] |
