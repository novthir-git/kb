...
## [2026-08-31] lint | 迁移到 GitHub 后体检：2 项确定性红（均为搬运损伤，已修）

本地目录经 zip 打包上传至云端会话，解压后建仓推送到 `novthir-git/kb`（`main` 与
`claude/push-local-to-repo-twyjkh` 两分支同容，127 文件）。随后按核心操作 3 全量体检。

**确定性检查**：重名文件 0；index 计数一致（页面声称 61 / 实际 61，素材声称 32 / 实际 30+2）；
wiki 页内 `raw/` 具体路径引用 33 条全部命中。

**判断类检查**：`overview.md` 57 行（≤ 60）；`topics/` 三页证据点表表头统一；61 页 frontmatter
必填字段齐全且 `sources` 均为块状列表；悬空双链仅 [[per-seat 定价的黄昏]] 与
[[system-of-record 与专有数据护城河]] 两条且均已标注待写；孤儿页 0；无"反复提及却缺页"的概念。
外链 194 个不同 URL，因同日早前条目已做全量复核，本次不重复。

**修复 1 — 根目录三个 symlink 退化为普通文件副本**（zip 打包解引用所致）。
`purpose.md`、`schema.md`、`CLAUDE.md` 变成了 `wiki/purpose.md`、`wiki/schema.md`、`AGENTS.md`
的独立字节副本。内容当时仍一致，但已构成铁律 7 所禁的第二事实源——本体日后修改，副本不会跟随，
正是 [[代码与文档漂移的本质]] 的表达漂移发生在 schema 自身。已删副本并重建为 symlink
（git 以 mode 120000 存储，推送后仍是链接）。属纯机械恢复，不 bump 任何页面的 `updated`。

**修复 2 — [[STORM-知识策展系统分析]] 内的 git 恢复指令在本仓库失效**。
该页降级说明里给出 `git show 88c8ad8:...` 用于取回已删除的知乎摘录存档，但 `88c8ad8` 只存在于
本机原始仓库；GitHub 副本由目录快照新建，无此历史对象。已就地加注限定说明，不删除原指令
（它在本机仍有效）。该页 `updated` 已是 2026-08-31，无需再 bump。

**结论**：wiki 本体无新增问题，两项均为跨机器搬运引入。后续若再以快照方式迁移，需注意
symlink 与 git 历史都不会随文件树过去。

## [2026-08-31] research | 守门判据显式化：知识的四条边界与「范围先于普遍性」

会话内沉淀（无联网）。起因：维护者把"快照迁移会丢 symlink 与 git 历史"这条操作教训判为值得
沉淀，用户以"我需要的是知识"驳回并追问判据；复盘产出的方法论增量写入 [[llm-wiki-方法论]]
新节「什么算『知识』——守门的四条边界」：

- 知识 = 经得起引用的判断（有出处、可被未来证据修正、改变后续决策）；
- 四条边界各对应一个 schema 机制：vs 信息（重推导成本）、vs 事件（衰减/复利）、
  vs 观点（可修正性）、vs 备忘（是否参与论断演化）；
- 第五条 purpose 相对性：普遍性只是知识的语法，purpose 才是语义——守门五问必须范围先于形状；
- 命名失败模式：把"有概括形状的陈述"当成知识（语法对了，语义为空）。

同步更新 [[llm-wiki-方法论]] 在 index 的描述行。未新建页面，页面计数不变（61）；
overview 不动（版图未变，一页变深）。

## [2026-09-16] research | Anthropic 自曝 CI 被自家 agent 出码量压垮：吞吐的下游代价

用户批准沉淀 09-15 简报的候选。回到一手来源核验（Anthropic 官方工程博客 2026-09-14，
作者 Sachin Malhotra），未采信简报转述。存档 `raw/sources/articles/2026-09-14-Anthropic-CI规模化压力.md`
（结构化摘录，非逐句镜像），新建 [[2026-09-14-Anthropic-CI规模化压力-源摘要]]（因被 4 个 wiki 页引用，
达到建源摘要页的门槛）。

核心事实：6 个月 CI job 涨 25×（人均季度出码 8×、其中 80% 由 Claude 撰写、测试总量 10×、
工程师人数仅 "nominal" 增加）；确定性的测试影响分析服务因"每测试一份运行历史 ⇒ 需单一写入者"
无法水平分片而被压垮；三次补丁分别撑 70 天 / 29 天 / 不到 1 天；架构重写单人 3 周完成，
原文明示新架构运行成本更高。

横向整合 4 页：

- [[Anthropic]]——"AI 原生软件工程"节追加证据与口径边界；标注 ⚠️ 矛盾：7 月源的"每日合并代码行 vs 2024 年 8×"
  与本源的"季度出码量 vs 2021–2025 8×"分母与基线都不同，不可互换叠加。
- [[agent-生产级落地的鸿沟]]——证据点表首行新增；与 Uber 行同构（卡点与解法都不在"模型更强"），
  但补出 Uber 没给的一面：规模化不免费。同时更新"待追踪"——三个规模化案例都只给过程/成本、
  不给质量结果，判为**系统性披露偏好**，应读作工程可行性证据而非净收益证据。
- [[Anthropic-AI原生SDLC治理循环]]——新增「吞吐的下游代价：CI 成为新瓶颈」一节，部分回答该页
  自列的证据空白 2（只回答成本侧，未回答价值侧）；指出治理循环隐含"CI 跑得动"这一未言明的依赖。
- [[生产力-体验悖论]]——新增「苦工的第三个去向：机器基建」，把负担迁移从"人→人"扩展到"人→机器账单"。

页面 61 → 62，素材 32 → 33。overview「落地节奏」线索行同步补注。
