...
| 调研稿论断 | 核验 | 校正后的口径 |
|---|---|---|
| arXiv 2606.05608 是"激进重构派"代表 | **基本成立** | 单作者预印本（15 页）、无自有实验、未经同行评审。v1（2026-06-04）题为 *The End of Software Engineering*；v2（2026-06-10）改题 *Agentic Software*，撤回"终结"改称"扩展"，但仍坚持 agent 范式是"复杂度缩放定律的必然结果"、AaaS 是"逻辑终点"——两版都属激进一端。"激进重构派"是调研稿的标签，作为该派"代表"证据分量偏弱，引用须注明版本 |
...
| 孤立任务成功率 >80%，持续演化 ≤38% | **失准** | 指标是综合 Score（新功能 Recall 与防回归 Precision 的调和），**不是成功率**，原文明确区别于二元成功口径（"成功率"的误称源自 AaaS 论文的图注）。持续演化最佳 38.03%（Claude Opus 4.6 + OpenHands）；按完全解决计最高 13.37%（Gemini 3 Pro）——证据比调研稿写的更强。独立任务的 >80% 部分是构造时校准出来的（标注者修订需求规格直到前沿模型独立运行达 80–90%） |
...
| 单一重流程不适配问题尺度；"宁愿 review 代码也不愿 review markdown" | **成立** | 基于 2025-09 版本工具；此后 Kiro 增加了 Quick Spec 与 Bugfix Specs，Spec Kit 增加了 bug fixing 与 idea assessment 入口 |
...
| "spec 是新源代码"的正方是 Tessl / Spec Kit / Kiro | **部分成立** | 旗手是 2025 年的 Tessl。GitHub 在 Spec Kit 自带的方法论文档里**理念上**接近 spec-as-source（代码是"持续再生成的输出"），但 Böckeler 判断工具实际形态只是 spec-first；其持久化文档引用了 Böckeler 的三层级，另列三种变更模型并声明都不是默认。Kiro 在 Böckeler 2025-09 的试用中"似乎基本是 spec-first"（厂商自称支持 spec 与代码同步）。反方 Böckeler 的"僵化 + 非确定性"是推测，原文没有"更糟"的断言 |
...
| 调研稿论断 | 核验 | 校正后的口径 |
|---|---|---|
| Anthropic 属于"代码库比 MCP / 子 agent 更根本"的收敛方 | **部分成立** | 一手材料支持"让代码库对 agent 可读（目录与命名、精简分层的 CLAUDE.md、代码地图）影响表现"，2026-05-14 的大型代码库文章也把"基础没做好就先接 MCP"列为常见错误；但 Anthropic 把表现归因于模型外的整个 harness，并**推荐**子 agent 架构，没有"代码库比 MCP / 子 agent 更根本"的主张 |
...
| 先在 2–3 个文件上调 prompt 再全量 fan-out；ratchet 式渐进收紧 | **部分成立**（复查自"基本成立"下调） | 可靠的部分是"少量文件试跑 → 按出错修 prompt → 全量铺开"：2–3 文件试跑是官方给自写 `claude -p` 循环的步骤（配合 `--allowedTools` 限权），Anthropic 的迁移实践也用少量文件做试跑；`/batch` 走自己的流程。"ratchet 式收紧"在所查 Anthropic 文档与博客中均无出处 |
