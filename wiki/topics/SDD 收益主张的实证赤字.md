...
| 2025-10-15 | Birgitta Böckeler《Understanding Spec-Driven-Development: Kiro, spec-kit, and Tessl》（martinfowler.com） | 2025-09 试用三款工具的定性观察：单一重流程不适配问题尺度；"宁愿 review 代码也不愿 review 一堆 markdown"；agent 常不遵循全部指令，偶尔又过度执行某条 constitution 条款——作者质疑 SDD 带来的是**"虚假的控制感"**；spec-as-source 可能同时继承 MDD 的僵化与 LLM 的非确定性 | 单人定性试用，无测量；基于 2025-09 版本工具（此后 Kiro 加了 Quick Spec，Spec Kit 加了 bug fixing 等入口）。作者认可 spec-first 的一般原则，但对当时 spec-first 工具的重流程与审查负担有保留；MDD 式的僵化加非确定性担忧针对 spec-anchored / spec-as-source。也是三层级分类的**最早提出者**（本库原把分类归于 arXiv 2602.00180，已更正）。与上一行 Scott Logic 的"审查负担"方向一致 |
