...
- [[Agentic SE 时代的系统重构]] — 代码库成为 agent 的运行环境：结构债可按 token 计价（单实验：同一变更输入 −83%），但须由 harness 驱动偿还、由验证网兜住；含 agent 友好代码库的目标形态、遗留系统方法与经核验的思想光谱。
...
## 主题（topics）
- [[agent-生产级落地的鸿沟]] — 论点：Agent 试验渗透显著高于规模化价值；证据点覆盖 Gartner、Deloitte、McKinsey、Meta 与方法论边界；08-31 增 Uber 规模化正向案例（其依赖项无一来自"模型更强"）；09-23 增 OpenAI 零手写代码实验、SWE-Milestone（独立任务 >80% → 持续演化最高 38%）与 Anthropic 内部调查（超过一半受访者可完全委派仅 0–20%）。
...
- [[2026-07-30-Thoughtworks-重构的经济收益-源摘要]] — 一次按 token 计价的重构实验（作者称是"第一步"）：17,155 行文件经 15 步重构，同一变更输入 token −83%；agent 不擅长选择与执行重构；含单次测量等六条口径。
