...
- **来源 URL**：https://www.uber.com/us/en/blog/efficient-software-factory/
- **作者**：Uday Kiran Medisetty（Distinguished Engineer, Uber）
- **发布日期**：2026-08-27
- **抓取日期**：2026-08-31
- **发现路径**：https://x.com/ubereng/status/2093444169037762840 （X 直连返回 402，正文取自 uber.com 原文）
- **存档性质**：正文文本提取自原页 HTML，保留全部正文段落、表格与图注文字。**图表（Figure 1–12）为图片，未存档**；
  凡"如图所示"的量化内容若未在正文文字中写出，本档不含该数值。段落顺序与小标题层级与原文一致。
...
|---|---|---|
| Portfolio | Total attributed cost; Distinct attributed users; Per-tool/agent cost, users, and spend share | Where the money is going, and which tool moved |
| Unit economics, per tool | Cost per user; Requests per user; Cost per 1,000 requests; Input, output, and total tokens per request; Cost per 1 million tokens; Cost per 1,000 sessions; Cost per active session hour; Prompt Cache hit rate | Whether the tool is getting cheaper or usage is just shifting |
| Model economics | For every model: Cost and cost share; Requests and request share; Cost per 1,000 requests; Cost per 1 million tokens | Which model releases actually changed the bill, at identical or different per-token prices |
| Driver decomposition | Cost change decomposed sequentially into adoption (users); engagement (requests per user); input workload (input tokens per request); output workload (output tokens per request) | Why the number moved, stated exactly, with nothing left in an unexplained residual |
| Managed agent outcomes | For each managed agent: Outcome-denominated cost (cost per merged PR, cost per review, cost per alert, cost per cleanup); Quality signal (revert rate, F1, MTTR); Volume (diffs landed, reviews posted, alerts triaged) | Whether each managed agent is getting cheaper per unit of value delivered, and whether quality holds through model migrations |
...
4. Looking ahead, we continually refine our workload performance by leveraging aggregated insights from our managed agents to test and deploy various model routing strategies.

（原文以"four steps"引出，但仅列出上述四段文字；第 4 条实为前瞻说明。存档保留原样。）
...
（原文致谢名单从略；含核心团队约 30 人及 Johannes Gehrke、Mattie Toia、Sumanth Sukumar、Praveen Neppalli Naga 等 leadership。）
